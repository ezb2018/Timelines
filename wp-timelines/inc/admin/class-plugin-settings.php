...
class WPEX_Timeline_Settings {
	...
	private function settings_fields() {
		...
		foreach ( $post_types  as $post_type ) {
			if($post_type->name!='attachment'&&$post_type->name!='wp-timeline'){
		...
		$settings['general'] = array(
			...
			'fields'				=> array(
				...
				array(
					'id' 			=> 'wpex_fontawesome_ver',
					'label'			=> esc_html__( 'Font Awesome version', 'wp-timeline' ),
					'description'	=> esc_html__( 'Choose Font Awesome version are you want to use. You may need enter full class of some icon again when switch from 4.7 to 5.3', 'wp-timeline' ),
					'type'			=> 'select',
					'options'		=> array( 
						'' => esc_html__( '4.7', 'wp-timeline' ),
						'5' => esc_html__( '5.3', 'wp-timeline' ),
					),
					'default'		=> ''
				),
				array(
				...
				array(
					'id' 			=> 'wpex_posttypes',
					'label'			=> esc_html__( 'Enable timeline metadata for custom post types', 'wp-timeline' ),
					'description'	=> esc_html__( 'This feature only effect to timeline shortcode, it will not change anything in single post', 'wp-timeline' ),
					'type'			=> 'checkbox_multi',
					'options'		=> $listpt,
					'default'		=> array()
				),
