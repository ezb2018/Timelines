...
class EXC_MB_Number_Field extends EXC_MB_Field {
	...
	public function html() { ?>
		...
		<input step="<?php echo esc_attr( $this->args['step'] ); ?>" type="number" <?php $this->id_attr(); ?> <?php $this->boolean_attr(); ?> <?php $this->class_attr( 'exc_mb_text_number code' ); ?> <?php $this->name_attr(); ?> value="<?php echo esc_attr( $this->value ); ?>" />
...
class EXC_MB_Date_Field extends EXC_MB_Field {
	...
	public function enqueue_scripts() {
		...
		wp_enqueue_style( 'exc_mb-jquery-ui', trailingslashit( EXC_MB_URL ) . 'css/vendor/jquery-ui/jquery-ui.css', '1.10.3' );
...
class EXC_MB_Date_Timestamp_Field extends EXC_MB_Field {
	...
	public function html() { ?>
		...
		<input <?php $this->id_attr(); ?> <?php $this->boolean_attr(); ?> <?php $this->class_attr( 'exc_mb_text_small exc_mb_datepicker' ); ?> type="text" <?php $this->name_attr(); ?>  value="<?php echo $this->value ? esc_attr( date( 'm\/d\/Y', $this->value ) ) : '' ?>" />
...
class EXC_MB_Datetime_Timestamp_Field extends EXC_MB_Field {
	...
	public function html() { ?>
		...
		<input <?php $this->id_attr('date'); ?> <?php $this->boolean_attr(); ?> <?php $this->class_attr( 'exc_mb_text_small exc_mb_datepicker' ); ?> type="text" <?php $this->name_attr( '[date]' ); ?>  value="<?php echo $this->value ? esc_attr( date( 'm\/d\/Y', $this->value ) ) : '' ?>" />
		<input <?php $this->id_attr('time'); ?> <?php $this->boolean_attr(); ?> <?php $this->class_attr( 'exc_mb_text_small exc_mb_timepicker' ); ?> type="text" <?php $this->name_attr( '[time]' ); ?> value="<?php echo $this->value ? esc_attr( date( 'h:i A', $this->value ) ) : '' ?>" />
...
class EXC_MB_Select extends EXC_MB_Field {
	...
	public function enqueue_scripts() {
		...
		wp_enqueue_script( 'select2-exthemes', trailingslashit( EXC_MB_URL ) . 'js/vendor/select2/select2.js', array( 'jquery' ) );
		wp_enqueue_script( 'field-select-exthemes', trailingslashit( EXC_MB_URL ) . 'js/field.select.js', array( 'jquery', 'select2-exthemes', 'exc_mb-scripts' ) );
	}
	...
	public function enqueue_styles() {
		...
		wp_enqueue_style( 'select2-exthemes', trailingslashit( EXC_MB_URL ) . 'js/vendor/select2/select2.css' );
	}
...
class EXC_MB_Group_Field extends EXC_MB_Field {
	...
	public function html() {
		...
		<?php if ( $this->args['repeatable'] ) : ?>
			<button class="exc_mb-delete-field">
				<span class="exc_mb-delete-field-icon">&times;</span>
				<?php echo esc_html( $this->args['string-delete-field'] ); ?>
			</button>
		<?php endif; ?>
