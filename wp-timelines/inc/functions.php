...
if(!function_exists('wpex_timeline_query')){
	function wpex_timeline_query($posttype, $count, $order, $orderby, $cat, $tag, $taxonomy, $meta_key, $ids,$page=false,$mult=false,$meta_value=false){
...
function ajax_wpex_loadmore_timeline(){
	...
	$back_p 		= isset($atts['back_p']) ? $atts['back_p'] : '';
	$page = $_POST['page'];
	...
	if($the_query->have_posts()){
		...
		while($the_query->have_posts()){ $the_query->the_post();
			...
			$html .= $content;
			$posttypes = get_option('wpex_posttypes');
...
function ajax_wpex_filter_taxonomy(){
	global $style, $ajax_load, $ID, $animations,$posttype,$show_media,$full_content,$feature_label,$lightbox,$hide_img,$img_size,$hide_title,$back_p;
	$ajax_load =1;
	$atts = json_decode( stripslashes( $_POST['param_shortcode'] ), true );
	$ID = isset($atts['ID']) ? $atts['ID'] : rand(10,9999);
	$posttype 		= isset($atts['posttype']) && $atts['posttype']!='' ? $atts['posttype'] : 'post';
...
if(!function_exists('wpex_custom_link')){
	function wpex_custom_link($back_p){
		...
		if($custom_link=='' && $wpex_disable_link!='yes'){
			$wpex_backbt = get_option('wpex_backbt');
			if($back_p!='' && is_numeric($back_p)){
				$custom_link = add_query_arg( 'bpage', $back_p, get_permalink(get_the_ID()) );
			}else{
...
if(!function_exists('wpextl_post_class')){
	add_filter('post_class', 'wpextl_post_class', 10,3);
	function wpextl_post_class($classes, $class, $post_id){
