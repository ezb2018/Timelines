...
<li <?php post_class($class);?> <?php echo 'data-id="filter-'.$ID.'_'.get_the_ID().'"';?>>
        ...
        <div class="wpex-timeline-label">
            <time class="wpex-timeline-time" datetime="<?php echo esc_attr(get_the_date( get_option( 'time_format' ) ).' '.get_the_date( get_option( 'date_format' ) ));?>">
                ...
                <a href="<?php echo $link_lb!='' ? $link_lb : $custom_link;?>" title="<?php the_title_attribute();?>">
                    <?php if(has_post_thumbnail(get_the_ID())){?><span class="ful-img"><?php the_post_thumbnail($img_size);?></span><?php }?>
                </a>
                <?php }?>
	...
	<style type="text/css">
		...
		.wpex-timeline-list.show-simple li.post-<?php the_ID();?> .wpex-timeline-icon .fa.no-icon:before{ background:url(<?php echo esc_url(wp_get_attachment_thumb_url( $wpex_icon_img ));?>); background-repeat: no-repeat; background-size: 100% auto; background-position: center;}
		<?php }
		if($we_eventcolor!=''){?>
		#timeline-<?php echo esc_attr($ID);?>.show-simple ul li.post-<?php the_ID();?> .wpex-timeline-time .tll-date{ background:<?php echo esc_attr($we_eventcolor);?>; color:#FFFFFF}
		.show-simple .wpex-timeline > li.post-<?php the_ID();?> .wpex-timeline-icon .fa{background:<?php echo esc_attr($we_eventcolor);?>;}
		.wpex-timeline-list.show-simple li.post-<?php the_ID();?> .wptl-readmore-center a,
		.wpex-timeline-list.show-simple li.post-<?php the_ID();?> .wpex-timeline-icon .fa:not(.no-icon):before{color:<?php echo esc_attr($we_eventcolor);?>;}
		<?php }?>
    </style>
    <?php } ?>
    <?php if($wpex_felabel!=''){
