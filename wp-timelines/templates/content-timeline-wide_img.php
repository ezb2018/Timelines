...
if($img_size==''){ $img_size = 'wptl-560x340';}
...
<li <?php post_class($class);?> <?php echo 'data-id="filter-'.$ID.'_'.get_the_ID().'"';?>>
        ...
        <div class="wpex-timeline-label">
            <div class="timeline-details">
                ...
                <div class="tlct-shortdes">
                    <?php 
					wptl_title_html($hide_title, $custom_link, 'style=" display:inline;"');
					if((get_post_meta( get_the_ID(), 'wpex_sublabel', true )!='')){
	...
	<style type="text/css">
		...
		#timeline-<?php echo esc_attr($ID);?> .post-<?php the_ID();?> .wpex-timeline-icon .fa.icon-img:before{ background:url(<?php echo esc_url(wp_get_attachment_thumb_url( $wpex_icon_img ));?>); background-repeat: no-repeat; background-size: 100% auto; background-position: center;}
		<?php }
		if($we_eventcolor!=''){?>
		#timeline-<?php echo esc_attr($ID);?> .post-<?php the_ID();?> .wpex-timeline-icon .fa,
        #timeline-<?php echo esc_attr($ID);?> .wpex-timeline > .post-<?php the_ID();?> .wpex-timeline-time span.tll-date{ background:<?php echo esc_attr($we_eventcolor);?>; color:#FFFFFF}
		#timeline-<?php echo esc_attr($ID);?>:not(.left-tl) .wpex-timeline > .post-<?php the_ID();?>:nth-child(odd) .wpex-timeline-time span.tll-date:before,
		#timeline-<?php echo esc_attr($ID);?>.left-tl .wpex-timeline > .post-<?php the_ID();?> .wpex-timeline-time span.tll-date:before{border-right-color:<?php echo esc_attr($we_eventcolor);?>}
		@media(min-width:768px){
			#timeline-<?php echo esc_attr($ID);?>:not(.left-tl) .wpex-timeline > .post-<?php the_ID();?>:nth-child(even) .wpex-timeline-time span.tll-date:before{ border-left-color:<?php echo esc_attr($we_eventcolor);?>;}
		}
		@media(max-width:767px){
			#timeline-<?php echo esc_attr($ID);?>:not(.left-tl) .wpex-timeline > .post-<?php the_ID();?>:nth-child(even) .wpex-timeline-time span.tll-date:before{ border-right-color:<?php echo esc_attr($we_eventcolor);?>;}
		}
		#timeline-<?php echo esc_attr($ID);?> .wpex-timeline > .post-<?php the_ID();?> .timeline-details{border-top-color:<?php echo esc_attr($we_eventcolor);?>;}
		#timeline-<?php echo esc_attr($ID);?>.left-tl .wpex-timeline > .post-<?php the_ID();?> .wpex-timeline-icon .fa:not(.no-icon):not(.icon-img):before{color:<?php echo esc_attr($we_eventcolor);?>;}
		<?php }?>
    </style>
    <?php } ?>
    <?php if($wpex_felabel!=''){
